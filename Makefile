SHELL := /bin/bash
.PHONY: build build-all run test test-coverage test-race test-all clean docker-build docker-run docker-stop logs help lint fmt vet race coverage bench install-tools ci staticcheck quality security deps-graph deps docs dev pre-commit release local-tls-certs

# Variables

BINARY_NAME=aero-arc-agent
DOCKER_IMAGE=aero-arc-agent
...
build:
	...
	@mkdir -p bin
	go build -o bin/$(BINARY_NAME) cmd/aero-arc-agent/main.go
...
build-all:
	...
	@mkdir -p bin
	GOOS=linux GOARCH=amd64 go build -o bin/$(BINARY_NAME)-linux-amd64 cmd/aero-arc-agent/main.go
	GOOS=darwin GOARCH=amd64 go build -o bin/$(BINARY_NAME)-darwin-amd64 cmd/aero-arc-agent/main.go
	GOOS=windows GOARCH=amd64 go build -o bin/$(BINARY_NAME)-windows-amd64.exe cmd/aero-arc-agent/main.go
...
deps-graph:
	...
	go-callvis -group pkg,type ./cmd/aero-arc-agent
...
logs:
	@echo "Viewing logs..."
	docker-compose logs -f aero-arc-agent
