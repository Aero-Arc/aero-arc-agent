SHELL := /bin/bash
.PHONY: build build-all run test test-coverage test-race test-all clean docker-build docker-run docker-stop logs help lint fmt vet race coverage bench install-tools ci staticcheck quality security deps-graph deps docs dev pre-commit release local-ssh-keys
